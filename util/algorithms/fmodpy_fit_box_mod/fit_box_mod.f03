...
CONTAINS
  SUBROUTINE compute_boxes(x_data, response, boxes, widths, weights)
    ...
    REAL (KIND=R8), INTENT(IN), DIMENSION(:,:) :: x_data
    ! The matrix of data points (stored in rows as x-vectors)

    REAL (KIND=R8), INTENT(IN), DIMENSION(UBOUND(x_data,2)) :: response
    ! The vector of response values associated with each point

    REAL (KIND=R8), INTENT(OUT), DIMENSION(:,:) :: boxes
    ! Holder for the center coordinates of each box-spline support region

    REAL (KIND=R8), INTENT(OUT), DIMENSION(UBOUND(boxes,2)) :: widths
    ! Holder for the width of each box-spline support region

    REAL (KIND=R8), INTENT(OUT), DIMENSION(UBOUND(boxes,2)) :: weights
    ...
    REAL (KIND=R8), DIMENSION(UBOUND(boxes,2),UBOUND(x_data,2)) :: coef_matrix
    ! The matrix of coefficients in the least squares solve, holds
    ! the evaluation of all box-splines at each x data point

    REAL (KIND=R8), DIMENSION(UBOUND(x_data,2)) :: local_response
    ! Local copy of 'response' so that the original is not tampered with

    REAL (KIND=R8), DIMENSION((UBOUND(x_data,2)*(UBOUND(x_data,2)-1))/2) :: distances
    ...
    REAL(KIND=R8), DIMENSION(2*UBOUND(x_data,1),UBOUND(boxes,2)) :: box_widths
  ...
  CONTAINS
    ...
    SUBROUTINE idx_to_pair(idx, num_individuals, first, second)
      ...
      pair_num = (1.0_R8 + SQRT(1.0_R8 + 8.0_R8 * i)) / 2.0_R8
      ...
      second = (pair_num - first) * first + 0.5_R8
  ...
  SUBROUTINE max_boxes(pts, box_widths)
    ...
    compute_box_shapes : DO step = 1,UBOUND(pts,2)
       ...
       calculate_box_width : DO other_pt = 1, UBOUND(pts,2)
          ...
          in_lower = (box_widths(:UBOUND(pts,1),step) .LT. 0.0_R8) .OR. &
               (pts(:,step)-pts(:,other_pt_idx) < box_widths(:UBOUND(pts,1),step))
          !  -- upper width undefined .OR. (other - box) < upper width
          in_upper = (box_widths(UBOUND(pts,1)+1:,step) .LT. 0.0_R8) .OR. &
  ...
  SUBROUTINE eval_box_coefs(boxes, widths, x_points, box_vals)
    ...
    REAL (KIND=R8), INTENT(IN), DIMENSION(:)   :: widths
    REAL (KIND=R8), INTENT(IN), DIMENSION(:,:) :: x_points
    REAL (KIND=R8), INTENT(OUT), DIMENSION(UBOUND(boxes,2)&
    ...
    box_evals : DO CONCURRENT (pt_num=1:UBOUND(x_points,2), &
       ...
       box_vals(box_num,pt_num) = PRODUCT(&
            MAX(1.0_R8 - shifted_box, 0.0_R8))

       ! ! Order 2 approximation
       ! shifted_box = 1.5_R8 + 1.5_R8 * shifted_box
       ! WHERE ((1.0_R8 .LE. shifted_box) .AND. &
       !      (shifted_box .LT. 2.0_R8)) shifted_box = (&
       !      -(2*shifted_box**2 - 6*shifted_box + 3) )
       ! WHERE ((2.0_R8 .LE. shifted_box) .AND. &
       !      (shifted_box .LE. 3.0_R8)) shifted_box = (&
       !      (shifted_box - 3)**2)
       ! WHERE (shifted_box .GT. 3.0_R8) shifted_box = 0.0_R8
  ...
  SUBROUTINE eval_boxes(boxes, widths, weights, x_points, response)
    REAL (KIND=R8), INTENT(IN), DIMENSION(:,:) :: boxes
    REAL (KIND=R8), INTENT(IN), DIMENSION(:)   :: widths
    REAL (KIND=R8), INTENT(IN), DIMENSION(:)   :: weights
    REAL (KIND=R8), INTENT(IN), DIMENSION(:,:) :: x_points
    REAL (KIND=R8), INTENT(OUT), DIMENSION(UBOUND(x_points,2))  :: response
    ! Local variables
    INTEGER :: x_pt
    REAL (KIND=R8), DIMENSION(UBOUND(boxes,2),UBOUND(x_points,2)) :: box_coefs
