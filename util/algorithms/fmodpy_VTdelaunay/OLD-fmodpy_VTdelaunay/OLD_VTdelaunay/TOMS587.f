...
C
     ...
     *), NEED LW=I1 BELOW, 70, NERR, IOPT, 1, LW, 0, 0,SNGL(DUMMY),
     *SNGL(DUMMY))
      MODE = 2
      RETURN
   10 CONTINUE
   20 IF (.NOT.(IWORK(2).GT.0)) GO TO 40
     ...
     **), NEED LIW=I1 BELOW, 72, NERR, IOPT, 1, LIW, 0, 0,SNGL(DUMMY),
     *SNGL(DUMMY))
      MODE = 2
      RETURN
   30 CONTINUE
...
C++
      ...
      DOUBLE PRECISION DIFF, DSQRT, DABS
      INTEGER   IP(N)
      ZERO = 0.D0
      FACTOR = 0.001D0
C
...
C     DATA IMACH(16)/  127 /
C
C     IEEE STANDARD MACHINE CONSTANTS
C 
      DATA IMACH( 1) /    5 /
      DATA IMACH( 2) /    6 /
      DATA IMACH( 3) /    0 /
...
99997 FORMAT (18H0LEAST SQS. RESID., D012.5, 2X, 12HFOR WNNLS( )/
     * 30H ERRORS, 1-X(I), FOR WNNLS( ).)
      WRITE (LOUT,99996) (I,X(I),I=1,N)
99996 FORMAT (4(I4, D012.5))
      GO TO 890
  880 CONTINUE
      WRITE (LOUT,99995) RNORML, IWORK(1), IWORK(2)
99995 FORMAT (18H0LEAST SQS. RESID., D012.5, 2X, 11HFOR LSEI( )/
...
C     MAKE THE MATRIX ORTHOGONAL BY SCALING THE ENTRIES.
 ...
 1080 GO TO NGO, (70, 110, 270, 310, 470, 510, 550)
      END
CC     PROGRAM CLSTST (INPUT,OUTPUT,TAPE5=INPUT,TAPE6=OUTPUT)            MAN   10
CC                                                                       MAN   20
CC     TO CONVERT THE CONSTRAINED LEAST SQUARES PACKAGE OF SUBPROGRAMS   MAN   30
CC     TO DOUBLE PRECISION, FOLLOW THE EDITING INSTRUCTIONS NEAR THE     MAN   40
CC     BEGINNING OF THE FOLLOWING SUBPROGRAMS                            MAN   50
CC     LSEI(), LSI(), LPDP(), WNNLS(), WNLSM(), WNLIT(),                 MAN   60
CC     HFTI(), H12(), DIFF(), AND THE TESTING SUBPROGRAM CLSTP().        MAN   70
CC     REVISED 820305-2000                                               MAN   80
CC     REVISED YYMMDD-HHMM                                               MAN   90
CC                                                                       MAN  100
CC                                                                       MAN  110
CC     CHANGE STRING /REAL             /DOUBLE PRECISION/, AND           MAN  120
CC     CHANGE STRING /E12/D12/                                           MAN  130
C      DOUBLE PRECISION COND(3)                                          MAN  140
C      INTEGER   KLOG(5)                                                 MAN  150
C      LOGICAL DONE                                                      MAN  160
CC                                                                       MAN  170
C      LIN = 5                                                           MAN  180
C      LOUT = I1MACH(4)                                                  MAN  190
CC                                                                       MAN  200
CC                                                                       MAN  210
CC     READ IN THE LOGS OF THE VARIOUS DIMENSIONS.                       MAN  220
C   10 READ (*  ,99999) (KLOG(I),I=1,5)                                  MAN  230
C99999 FORMAT (5I5)                                                      MAN  240
C      DONE = KLOG(5).LT.0                                               MAN  250
C      IF (.NOT.(DONE)) GO TO 20                                         MAN  260
C      GO TO 30                                                          MAN  270
CC                                                                       MAN  280
CC     READ THE CONDITION NUMBERS FOR THE THREE MATRICES.                MAN  290
C   20 READ (*  ,99998) (COND(I),I=1,3)                                  MAN  300
C99998 FORMAT (3F10.0)                                                   MAN  310
CC                                                                       MAN  320
C      WRITE (LOUT,99997) KLOG, COND                                     MAN  330
C99997 FORMAT (/////17H KLOG(*) ARRAY = , 5I5/17H COND(*) ARRAY = ,      MAN  340
C     * 3D12.4)                                                          MAN  350
CC                                                                       MAN  360
CC     CALL TESTING SUBPROGRAM TO FORM CONSTRAINED LEAST SQUARES         MAN  370
CC     SYSTEM AND SOLVE IT.                                              MAN  380
C      CALL CLSTP(KLOG, COND, ISTAT)                                     MAN  390
CC                                                                       MAN  400
C      WRITE (LOUT,99996) ISTAT                                          MAN  410
C99996 FORMAT (/23H FROM CLSTP( ), ISTAT =, I2, 2X, 16H(ISTAT=4 IS GOOD/ MAN  420
C     * 27X, 24H ISTAT=1,2,3 MAY BE BAD))                                MAN  430
CC                                                                       MAN  440
C      GO TO 10                                                          MAN  450
C   30 STOP                                                              MAN  460
C      END                                                               MAN  470
