...
EXTERNAL :: Project
! allocate data
ALLOCATE(A(d,d), b(d), currCenter(d), LQ(d,d), piv(d), &
...
IF (k > budget) err = 2
! free heap
DEALLOCATE(A, b, currCenter, LQ, piv, plane, proj, work)
! done
RETURN
! end subroutine
...
simp(d+1) = 0
currCenter = REAL(0.,R8)
currRad = HUGE(currRad)
! loop through all points
DO i = 1, n
        ! check point(i) is inside current ball
        IF (DNRM2(d, pts(:,i) - currCenter(:), 1) > currRad) CYCLE
        ...
        currRad = DNRM2(d, currCenter, 1)
        currCenter = currCenter + pts(:,simp(1))
        simp(d+1) = i
END DO
! reset error flag
err = 0
