...
MODULE PLRM
  ...
  REAL :: DGEMM_TIMER, WHERE_TIMER, START_TIME, STOP_TIME
...
CONTAINS
  ...
  CONTAINS
    ...
    SUBROUTINE EVALUATE_BATCH()
      ...
      END WHERE
      CALL CPU_TIME(STOP_TIME)
      WHERE_TIMER = WHERE_TIMER + (STOP_TIME - START_TIME)
      ! Compute the next set of internal values with a rectified activation.
      DO L = 1, MNS-1
         LP1 = L+1
         CALL CPU_TIME(START_TIME)
  ...
  SUBROUTINE MINIMIZE_MSE(X, Y, STEPS, BATCH_SIZE, NUM_THREADS, &
    ...
    ! Local variables.
    REAL(KIND=REAL64), PARAMETER :: STEP_FACTOR = 2_REAL64 ** (-3)
    ...
    DO I = 1, S
       ...
       !$OMP&  REDUCTION(+: DGEMM_TIMER, WHERE_TIMER, &
