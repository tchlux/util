...
PROGRAM TEST
  ...
  IMPLICIT NONE
  ! Global settings.
  INTEGER, PARAMETER :: N = 2**11, DI = 2**8, DO = 1
  REAL(KIND=REAL32), PARAMETER :: PI = 3.1415926535897932
  INTEGER, PARAMETER :: DS = 64, NS = 8, STEPS = 200
  ! Local variables.
  ...
  CALL MINIMIZE_MSE(X(:,:), Y(:,:), STEPS=STEPS, MEAN_SQUARED_ERROR=MSE, &
       NUM_THREADS=4, DROPOUT=0.0)
  CALL MINIMIZE_MSE(X(:,:), Y(:,:), STEPS=10, MEAN_SQUARED_ERROR=MSE, &
       NUM_THREADS=1)
  CALL STOP_TIMER()
  WRITE (*,'("  ",1F10.6,/)') TOTAL()
  WRITE (*,*) MSE
