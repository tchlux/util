...
CONTAINS
  ...
  SUBROUTINE INIT_MODEL(INPUTS, OUTPUTS, SEED)
    ...
    REAL(KIND=REAL32), DIMENSION(:,:), ALLOCATABLE :: INTERNAL_VALUES, OUTPUT_VALUES
  ...
  SUBROUTINE SWAP_PARAMS(L, N1, N2, &
    ...
    ! Model parameters.
    REAL(KIND=REAL32), INTENT(INOUT), DIMENSION(:,:)   :: INPUT_PARAMS
    REAL(KIND=REAL32), INTENT(INOUT), DIMENSION(:)     :: INPUT_BIAS
    REAL(KIND=REAL32), INTENT(INOUT), DIMENSION(:,:,:) :: INTERNAL_PARAMS
    REAL(KIND=REAL32), INTENT(INOUT), DIMENSION(:,:)   :: INTERNAL_BIAS
    REAL(KIND=REAL32), INTENT(INOUT), DIMENSION(:,:)   :: OUTPUT_PARAMS
    REAL(KIND=REAL32), INTENT(INOUT), DIMENSION(:)     :: OUTPUT_BIAS     
    ! Local variables.
    INTEGER :: I
  ...
  SUBROUTINE EVALUATE(INPUTS, OUTPUTS)
    ...
    ! Internal values.
    REAL(KIND=REAL32), DIMENSION(:,:), ALLOCATABLE :: INTERNAL_VALUES
    ! Compute the number of nodes (to know where "bias" value is).
    INTEGER :: D, L
    ...
    END DO
    ! Compute the output.
    CALL SGEMM('T', 'T', MDO, SIZE(INPUTS,2), MDS, 1.0_REAL32, &
         OUTPUT_PARAMS(:,:), SIZE(OUTPUT_PARAMS,1), &
         INTERNAL_VALUES(:,:), SIZE(INTERNAL_VALUES,1), &
  ...
  SUBROUTINE MINIMIZE_MSE(X, Y, STEPS, BATCH_SIZE, NUM_THREADS, &
    ...
    ELSE
       BS = (NX + NT - 1) / NT
       ! TODO: There are large enough point sets that cause seg-faults.
       ! BS = MIN((NX + NT - 1) / NT, )
       ! PRINT *, 'biggest:', MAX(MDS*MNS*BS, MDO*BS)
    END IF
    ...
    fit_loop : DO I = 1, S
       ...
       P1_GRAD(:,1:MDS)   = 0.0_REAL32
       B1_GRAD(1:MDS)     = 0.0_REAL32
       P2_GRAD(:,1:MDS,:) = 0.0_REAL32
       B2_GRAD(1:MDS,:)   = 0.0_REAL32
       P3_GRAD(1:MDS,:)   = 0.0_REAL32
       B3_GRAD(:)         = 0.0_REAL32
