...
CONTAINS
  ...
  SUBROUTINE SSE_GRADIENT(INPUTS, OUTPUTS, SUM_SQUARED_ERROR, &
    ...
    LOGICAL, DIMENSION(MDS,SIZE(INPUTS,2)) :: ZEROS
    ! Number of points.
    REAL(KIND=REAL64) :: N
    INTEGER :: D
  ...
  CONTAINS
    ...
    SUBROUTINE EVALUATE_BATCH()
      ...
      CALL DGEMM('N', 'N', MDS, SIZE(INPUTS,2), MDI, 1.0_REAL64, &
      ...
      DO L = 1, MNS-1
         LP1 = L+1
         ! INTERNAL_VALUES(:,:,LP1) = MATMUL(INTERNAL_PARAMS(:,:,L), INTERNAL_VALUES(:,:,L))
         CALL DGEMM('N', 'N', MDS, SIZE(INPUTS,2), MDS, 1.0_REAL64, &
      ...
      CALL DGEMM('N', 'N', MDO, SIZE(INPUTS,2), MDS, 1.0_REAL64, &
    ...
    SUBROUTINE GRADIENT_BATCH()
      ...
      OUTPUT_BIAS_GRADIENT(:) = SUM(OUTPUT_GRADIENT(:,:), 2) / N &
           + OUTPUT_BIAS_GRADIENT(:)
      ! Compute output parameter gradients.
      ! OUTPUT_PARAMS_GRADIENT(:,:) = MATMUL( OUTPUT_GRADIENT(:,:), &
      !      TRANSPOSE(INTERNAL_VALUES(:,:,MNS)) ) / N &
      !      + OUTPUT_PARAMS_GRADIENT(:,:)
      CALL DGEMM('N', 'T', MDO, MDS, SIZE(INPUTS,2), 1.0_REAL64 / N, &
      ...
      CALL DGEMM('T', 'N', MDS, SIZE(INPUTS,2), MDO, 1.0_REAL64, &
           OUTPUT_PARAMS(:,:), SIZE(OUTPUT_PARAMS,1), &
           OUTPUT_GRADIENT(:,:), SIZE(OUTPUT_GRADIENT,1), &
      ...
      INTERNAL_REPRESENTATIONS : DO L = MNS-1, 1, -1
         ...
         INTERNAL_BIAS_GRADIENT(:,L) = SUM(INTERNAL_VALUES(:,:,LP1), 2) / N &
              + INTERNAL_BIAS_GRADIENT(:,L)
         ! Compute the gradient with respect to each output and all inputs.
         ! INTERNAL_PARAMS_GRADIENT(:,:,L) = MATMUL( &
         !      INTERNAL_VALUES(:,:,LP1), TRANSPOSE(INTERNAL_VALUES(:,:,L))) / N &
         !      + INTERNAL_PARAMS_GRADIENT(:,:,L)
         CALL DGEMM('N', 'T', MDS, MDS, SIZE(INPUTS,2), 1.0_REAL64 / N, &
         ...
         CALL DGEMM('T', 'N', MDS, SIZE(INPUTS,2), MDS, 1.0_REAL64, &
              INTERNAL_PARAMS(:,:,L), SIZE(INTERNAL_PARAMS,1), &
              INTERNAL_VALUES(:,:,LP1), SIZE(INTERNAL_VALUES,1), &
  ...
  SUBROUTINE MINIMIZE_MSE(X, Y, STEPS, MAX_BATCH_SIZE, MEAN_SQUARED_ERROR)
    ...
    ! Internal values and lists of which ones are nonzero.
    REAL(KIND=REAL64), DIMENSION(MDS,SIZE(X,2),MNS) :: INTERNAL_VALUES
    !  storage for the gradient of the error function
    REAL(KIND=REAL64), DIMENSION(MDO,SIZE(X,2)) :: OUTPUT_GRADIENT
    !  local integers
