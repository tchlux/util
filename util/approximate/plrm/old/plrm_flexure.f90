...
CONTAINS
  ...
  SUBROUTINE EVALUATE(INPUTS, OUTPUTS, NUM_THREADS, INTERNAL_POSITIONS)
    REAL(KIND=RT), INTENT(IN), DIMENSION(:,:) :: INPUTS
       ...
       DO D = 1, MDS
          ...
          IF (PRESENT(INTERNAL_POSITIONS)) THEN
             WHERE (INTERNAL_VALUES(BS:BE,D) .LT. INPUT_MIN(D))
                INTERNAL_POSITIONS(BS:BE,D,1) = 0
             ELSEWHERE (INTERNAL_VALUES(BS:BE,D) .LT. DISCONTINUITY)
                INTERNAL_POSITIONS(BS:BE,D,1) = 1
             ELSEWHERE (INTERNAL_VALUES(BS:BE,D) .LE. INPUT_MAX(D))
                INTERNAL_POSITIONS(BS:BE,D,1) = 2
             ELSEWHERE
                INTERNAL_POSITIONS(BS:BE,D,1) = 3
             END WHERE
          END IF
          ...
          WHERE (INTERNAL_VALUES(BS:BE,D) .LT. DISCONTINUITY)
             INTERNAL_VALUES(BS:BE,D) = INTERNAL_VALUES(BS:BE,D) * INPUT_FLEX(D)
          END WHERE
       END DO
       ! Compute the next set of internal values with a rectified activation.
       DO L = 1, MNS-1
          ...
          DO D = 1, MDS
             ...
             IF (PRESENT(INTERNAL_POSITIONS)) THEN
                WHERE (INTERNAL_VALUES(BS:BE,D) .LT. INTERNAL_MIN(D,L))
                   INTERNAL_POSITIONS(BS:BE,D,L+1) = 0
                ELSEWHERE (INTERNAL_VALUES(BS:BE,D) .LT. DISCONTINUITY)
                   INTERNAL_POSITIONS(BS:BE,D,L+1) = 1
                ELSEWHERE (INTERNAL_VALUES(BS:BE,D) .LE. INTERNAL_MAX(D,L))
                   INTERNAL_POSITIONS(BS:BE,D,L+1) = 2
                ELSEWHERE
                   INTERNAL_POSITIONS(BS:BE,D,L+1) = 3
                END WHERE
             END IF
             ! Clip the values based on the observerd minimums and maximums.
             ...
             ! Apply the flexure function.
             WHERE (INTERNAL_VALUES(BS:BE,D) .LT. DISCONTINUITY)
                INTERNAL_VALUES(BS:BE,D) = INTERNAL_VALUES(BS:BE,D) * INTERNAL_FLEX(D,L)
             END WHERE
          END DO
       END DO
