...
!      singular value decomposition.
! 89 : The LAPACK subroutine DORMQR has reported an illegal value.
!
! Optional arguments:
!
...
REAL(KIND=R8) :: U(1,1), VT(1,1) ! Dummy arrays that will not be initialized.
...
EXTERNAL :: DGETRF ! Perform a LU factorization (LAPACK).
...
! Query DGESVD and DGEQP3 for optimal work array size (LWORK).
LWORK = -1
CALL DGESVD('N','N',D,D,LQ,D,PLANE(1:D),U,1,VT,1,B,LWORK,IERR(1))
CALL DGEQP3(D-1,D,LQ,D,IPIV,PLANE,CENTER,LWORK,IERR(1))
LWORK = MAX(INT(CENTER(1)), INT(B(1))) ! Compute the optimal work array size.
...
! This subroutine depends on a variation for handling degenerate or
...
DO I = 2, D
   ...
   DO J = 1, N
      ...
      LQ(:,1:I) = A(:,1:I)
      ! Compute A^T P = Q R.
      CALL DGEQP3(D, I, LQ, D, IPIV, PLANE, WORK, LWORK, IERR(MI))
      IF(IERR(MI) < 0) THEN ! LAPACK illegal input error.
         IERR(MI) = 84; RETURN
      ELSE IF (IERR(MI) > 0 .OR. ABS(LQ(I,I)) < EPSL) THEN ! Rank-deficient.
      ...
      CENTER(I+1:D) = 0
      CALL DORMQR('L', 'N', D, 1, I, LQ, D, PLANE, CENTER, D, WORK, LWORK, &
         IERR(MI))
      IF(IERR(MI) < 0) THEN ! LAPACK illegal input error.
         IERR(MI) = 89; RETURN
      END IF
...
! SIMPS(1,MI) and SIMPS(2,MI) have already been selected, and A^T(:,1) and
! B(1) have been filled appropriately.
!
! On output, this subroutine fully populates the matrix A^T and vector B,
! and fills SIMPS(:,MI) with the indices of a valid Delaunay simplex. This
...
DO I = 2, D
   ...
   DO J = 1, N
      ...
      LQ(:,1:I) = A(:,1:I)
      CENTER(1:I) = B(1:I)
! *** NEED TO REPLACE THIS WITH TRANSPOSE *** !
...
! factorization of A:  AP = QR, where P permutes the columns of A.  Then the
! last column of Q is orthonormal to the range of A.
! 
...
!   CALL DGEQP3(D-1,D,LQ,D,IPIV,PLANE,WORK,LWORK,IERR(MI))
   CALL DGEQP3(D,D-1,LQ,D,IPIV,CENTER,WORK,LWORK,IERR(MI))
   IF(IERR(MI) < 0) THEN ! LAPACK illegal input error.
      IERR(MI) = 84; RETURN
   ELSE IF (IERR(MI) > 0) THEN ! Rank-deficiency detected.
      IERR(MI) = 61; RETURN
...
!   PLANE(1:D) = PLANE(1:D) / DNRM2(D,PLANE(1:D),1)
   ...
   PLANE(1:D-1) = 0.0_R8
   PLANE(D) = 1.0_R8
   CALL DORMQR('L', 'N', D, 1, D-1, LQ, D, CENTER, PLANE, D, WORK, &
    LWORK, IERR(MI))
   IF(IERR(MI) < 0) THEN ! LAPACK illegal input error.
      IERR(MI) = 89; RETURN
   END IF
