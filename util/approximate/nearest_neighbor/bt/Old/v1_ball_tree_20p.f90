...
CONTAINS
  ...
  RECURSIVE SUBROUTINE BUILD_TREE(POINTS, RADII, SQ_SUMS, ROOT, &
    ...
    DO I = 1, SIZE(POINTS,2) ; PT_INDICES(I) = I ; END DO
  ...
  RECURSIVE SUBROUTINE PT_NEAREST(POINT, K, TREE, RADII, SQ_SUMS, &
       LEAF_SIZE, INDICES, DISTS, PT_SS, FOUND, MAX_IDX, START_IDX)
    ...
    INTEGER(KIND=INT64), INTENT(INOUT), OPTIONAL :: FOUND, MAX_IDX
    INTEGER(KIND=INT64), INTENT(IN),    OPTIONAL :: START_IDX
    ! Local variables
    INTEGER(KIND=INT64) :: F, MI, SI, I, I1, I2
    REAL(KIND=REAL64)   :: D, D1, D2, PS
    ! Initialize FOUND, MAX_IDX, and START_IDX values for first call,
    ! if FOUND is present then this must not be first and all are present.
    ...
    ! If (FOUND .EQ. 0) then this is the first call, measure dist to root.
    IF (F .EQ. 0_INT64) THEN
       INDICES(1) = 1_INT64
       ...
       INDICES(2:) = 0_INT64
       ! Initialize values for FOUND, MAX_IDX, and START_IDX.
       F = 1_INT64
       MI = 1_INT64
       SI = 0_INT64
    END IF
    ...
    BRANCH_OR_LEAF : IF (SIZE(TREE,2) .GT. LEAF_SIZE) THEN
       ...
       DIST_CALCS = DIST_CALCS + 1
       ! If we have not found K points yet, then automatically add this
       ! point to the list of closest-K points, update max distance.
       UPDATE_MAX_DIST1 : IF (F .LT. K) THEN
          F = F + 1_INT64
          DISTS(F) = D1
          INDICES(F) = I1 + SI
          IF (D1 .GT. DISTS(MI)) MI = F
          ! Otherwise, only store this point if its distance is less than
          ! that of the furthest point found so far. On update, calculate
          ! the next furthest distance value and store.
       ELSE IF (D1 .LT. DISTS(MI)) THEN
          DISTS(MI) = D1
          INDICES(MI) = I1 + SI
          MI = MAXLOC(DISTS,1)
       ...
       DIST_CALCS = DIST_CALCS + 1
       UPDATE_MAX_DIST2 : IF (F .LT. K) THEN
          F = F + 1_INT64
          DISTS(F) = D2
          INDICES(F) = I2 + SI
          IF (D2 .GT. DISTS(MI)) MI = F
       ELSE IF (D2 .LT. DISTS(MI)) THEN
          DISTS(MI) = D2
          INDICES(MI) = I2 + SI
          MI = MAXLOC(DISTS,1)
       ...
       INNER_CHILD_CLOSER : IF (D1 .LE. D2) THEN
          ...
          SEARCH_INNER1 : IF ((F .LT. K) .OR. &
             ...
             CALL PT_NEAREST(POINT, K, TREE(:,I1:I), RADII(I1:I), &
                  SQ_SUMS(I1:I), LEAF_SIZE, INDICES, DISTS, PS, F, MI, SI+1)
          ...
          SEARCH_OUTER1 : IF ((F .LT. K) .OR. &
             ...
             CALL PT_NEAREST(POINT, K, TREE(:,I2:), RADII(I2:), &
                  SQ_SUMS(I2:), LEAF_SIZE, INDICES, DISTS, PS, F, MI, SI+I2-1)
    ...
    ELSE
       DIST_TO_CHILDREN : DO I = 2, SIZE(TREE,2)
          ...
          DIST_CALCS = DIST_CALCS + 1
          UPDATE_MAX_DIST3 : IF (F .LT. K) THEN
             F = F + 1_INT64
             DISTS(F) = D
             INDICES(F) = I + SI
             IF (D .GT. DISTS(MI)) MI = F
          ELSE IF (D .LT. DISTS(MI)) THEN
             DISTS(MI) = D
             INDICES(MI) = I + SI
             MI = MAXLOC(DISTS,1)
    ...
    SORT_K : IF (PRESENT(FOUND)) THEN
       ! This is not the root, We need to pass the updated values of
       ! FOUND and MAX_IDX back up the recrusion stack.
       FOUND = F
       MAX_IDX = MI
    ELSE
